...
function srgbToOklab(r: number, g: number, b: number) {
  ...
  return [0.2104542553 * l + 0.793617785 * m - 0.0040720468 * s, 1.9779984951 * l - 2.428592205 * m + 0.4505937099 * s, 0.0259040371 * l + 0.7827717662 * m - 0.808675766 * s] as const;
}
...
function oklabToSrgb(L: number, A: number, B: number) {
  ...
  return [toGamma(4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * s), toGamma(-1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s), toGamma(-0.0041960863 * l - 0.7034186147 * m + 1.707614701 * s)] as const;
}
...
export default function LineCursor(props: Partial<LineCursorProps>) {
  ...
  useEffect(() => {
    ...
    function onFirstMove(e: MouseEvent | TouchEvent) {
      moveTarget.removeEventListener("mousemove", onFirstMove as EventListener);
      moveTarget.removeEventListener("touchstart", onFirstMove as EventListener);
      moveTarget.addEventListener("mousemove", updatePosition as EventListener);
      moveTarget.addEventListener("touchmove", updatePosition as EventListener, { passive: false });
      updatePosition(e);
      buildLines();
      started = true;
      loop();
    ...
    function handleFocus() {
      if (!running) { running = true; if (started) loop(); }
    }
    ...
    moveTarget.addEventListener("touchstart", onFirstMove as EventListener, { passive: true });
    ...
    return () => {
      ...
      moveTarget.removeEventListener("mousemove", onFirstMove as EventListener);
      moveTarget.removeEventListener("touchstart", onFirstMove as EventListener);
      moveTarget.removeEventListener("mousemove", updatePosition as EventListener);
      moveTarget.removeEventListener("touchmove", updatePosition as EventListener);
      ro?.disconnect();
